...
pp_plot <- ggplot(df, aes(x=loss.percentage, y=avg / 1000)) +
           geom_line() +
           geom_errorbar(data=df, mapping=aes(x=loss.percentage, ymin=lower_percentile / 1000, ymax=upper_percentile / 1000), width=0.2) + # , size=0.3
           ...
           labs(x="Packet Loss [%]", y="Time [s]")
ggsave("pingpong.pdf", plot=pp_plot, width=3.4, height=2.3)
